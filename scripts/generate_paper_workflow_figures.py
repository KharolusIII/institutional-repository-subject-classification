...
def experimental_workflow(output: Path):
    ...
    box(ax, 0.22, 0.38, 0.25, 0.22, "Frozen dense route",
        "SBERT · LaBSE\nOverlapping token chunks\nLength-weighted hierarchical pooling",
        face=LIGHT_BLUE, edge=BLUE)
    box(ax, 0.22, 0.10, 0.25, 0.22, "Fine-tuned route",
        "SBERT · LaBSE\n≤ 8 non-overlapping chunks per text unit\nWeighted loss and document logits",
    ...
    ax.text(0.34, 0.035, "233 validation candidates (189 sparse + 42 frozen dense + 2 fine-tuned)",
            ha="center", fontsize=9.5, color=GRAY)
    ax.text(0.635, 0.035, "Dedicated calibration split", ha="center", fontsize=9.5, color=GRAY)
    ax.text(0.885, 0.035, "2,959 frozen test items", ha="center", fontsize=9.5, color=GRAY)
    finish(fig, ax, output)
